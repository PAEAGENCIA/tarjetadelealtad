...
const { safeEqual } = require('./businesses');
...
// Solo el dueño de la tarjeta (con la clave de su liga, ?t=) puede guardarla en su Wallet.
function getBusinessAndCustomer(slug, phone, token) {
  ...
  if (!customer || customer.status === 'pending') return { error: 'No encontramos esa tarjeta.', status: 404 };
  if (!customer.card_token || !safeEqual(token, customer.card_token)) return { error: 'No encontramos esa tarjeta.', status: 404 };
...
router.get('/apple/:phone', async (req, res) => {
  const { business, customer, error, status } = getBusinessAndCustomer(req.params.slug, req.params.phone, req.query.t || req.header('x-card-token'));
